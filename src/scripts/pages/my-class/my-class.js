...
export default class MyClass {
  ...
  populateMyListModules(message, courses) {
    ...
    `;
    courses.map((course) => {
      percentageIndicator(document.querySelector(`#progress-${course.courseId}`), document.querySelector(`#percentage-${course.courseId}`), course.progress, document.querySelector(`[data-courseId="${course.courseId}"] .card-btn`));
    });
  }
