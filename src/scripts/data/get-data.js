...
export const DataLearning = {
  ...
  async updateModuleToComplete(courseId, moduleId) {
    ...
    if (courseIndex !== -1) {
      const moduleIndex = learningData.data[courseIndex].modules.findIndex(
      ...
      if (moduleIndex !== -1) {
        ...
        const completedModules = learningData.data[courseIndex].modules.filter(
          (m) => m.completed,
        ).length;
        learningData.data[courseIndex].progress =
          totalModules > 0 ? Math.round((completedModules / totalModules) * 100) : 0;
        if (
          learningData.data[courseIndex].progress === 100 &&
